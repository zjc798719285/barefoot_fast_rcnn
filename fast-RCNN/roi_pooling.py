...
def ROI_pooling(fc_map, roi, out_size):
    ...
    roi_row = tf.maximum(tf.cast(shape_fc_map[1] * tf.gather(indices=0, params=roi)[0], tf.int32), 1)
    roi_col = tf.maximum(tf.cast(shape_fc_map[2] * tf.gather(indices=1, params=roi)[0], tf.int32), 1)
